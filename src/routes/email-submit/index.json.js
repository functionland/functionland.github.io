	...
	try {
		...
		if (res.status == 200 || res.status == 201) {
			...
			if (res.status == 204){
				...
				console.log("Docs guide: The server fulfilled the request but does not need to return an entity-body");
			}
			if (res.status == 400) {
				console.log("Bad request");
				console.log("Docs guide: The request could not be understood by the server due to malformed syntax.");
			}
			if (res.status == 401) {
				console.log("Bad request");
			...
			if (res.status == 404) {
				console.log("Bad request");
				console.log("Docs guide: The server has not found anything matching the Request-URI. No indication is given of whether the condition is temporary or permanent.");
			}
			if (res.status == 500) {
				console.log("Bad request");
				console.log("Docs guide: The server encountered an unexpected condition which prevented it from fulfilling the request.");
			}
